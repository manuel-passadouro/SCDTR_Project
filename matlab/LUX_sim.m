...
hold on;
plot(V_led, LUX_b_nom, '-o', 'LineWidth', 2, 'DisplayName', 'Nominal');
%plot(V_led, LUX_b_max, '-o', 'LineWidth', 2, 'DisplayName', 'Original LUX');
plot(V_led, LUX_new, '-x', 'LineWidth', 2, 'DisplayName', 'Ajusted');
xlabel('LED Voltage (V)');
ylabel('LUX');
title('LUX vs. LED Voltage');
legend('show');
grid on;

% Log-log plot of LUX vs. R_LDR
figure;
loglog(LUX_b_min, R_ldr, '-o', 'LineWidth', 2, 'DisplayName', 'LUX vs. R_{LDR} Upper');
hold on;
loglog(LUX_b_nom, R_ldr, '-o', 'LineWidth', 2, 'DisplayName', 'LUX vs. R_{LDR} Nominal');
loglog(LUX_b_max, R_ldr, '-o', 'LineWidth', 2, 'DisplayName', 'LUX vs. R_{LDR} Lower');
loglog(LUX_new, R_ldr, '-x', 'LineWidth', 2, 'DisplayName', 'New LUX vs. R_{LDR}');
xlabel('LUX');
ylabel('R_{LDR} (Ω)');
title('Log-Log Plot of LUX vs. R_{LDR}');
legend('show');
grid on;
